...
I_i = -0.2
time_steps = 12000
dt = 0.01 #is actually Dt and is used as the delta_t in my model when integrating
d = 0.001
...
targ_data = adjmat[:,:,10,3] #choosing one eopch instead of averaging
targ_data_p = matrix2p(targ_data)

tract_mat = np.zeros((nodes,nodes))

#%%
def plot_avg_pli(ts, len_chunk, timesteps, fmin= 17, fmax =20):
    ue_targ_chunks= []
    ...
    for i in range(num_chunks):
        all_ue_pli.append(_con[:,:,0]) #2 corresponds to freq =17
...
def residuals_pli(p, final = False):
    ...
    if final:
        ...
        plot_mat(targ_data, "targ corr", _max = upper)
