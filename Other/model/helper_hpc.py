...
def plot_pwr_spec(data_array, nodes, dt = 0.01, length = all):
    for i in range(nodes):
...
def plot_mat(matrix, title: str = "title", max: int = 1, min: int = 0,
             exclude = None):
    """
    ...
    fig, ax = plt.subplots()
    plt.title(title)
    cax = ax.imshow(copy, aspect='auto', vmax = max, vmin = min)
    cbar = fig.colorbar(cax)
    plt.show()
    print(copy)