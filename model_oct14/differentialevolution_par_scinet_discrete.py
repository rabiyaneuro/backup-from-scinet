...
#            np.save("before_rank"+str(self.rank)+"iter"+str(nit), self.population)
            ...
            print("de step {}: f(x)= {}, comp time: {} RANK: {}"
                  .format(nit, self.population_energies[0], time.time()-t,
                  self.rank))
...
#                     np.empty(1, dtype=np.float64)]
        ...
        print('rank ', self.rank, 'unix time ', time.time())
        self.replace_from_mig(data_recv[0], data_recv[1])
        
        print("RANK in migr: {}, comm time sec: {} ".format(self.rank, 
              time.time()-t))
        return
...
#                print("creating trial solution")
            ...
            i = 0
            while not self._ensure_constraint(trial) and i <5:
                trial = self._mutate(candidate)
                i = i +1
    ...
    def _ensure_constraint(self, trial, meth = "ref"):
        """
